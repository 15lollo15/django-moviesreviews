...
class PersonTest(TestCase):
    ...
    DIRECTOR_NAME = "Luigi Verdi"

    def test_is_an_actor_with_no_movies_as_actor(self):
        non_actor = Person(birthday=timezone.now(), name_surname=PersonTest.ACTOR_NAME)
        non_actor.save()
    ...
    def test_is_an_actor_with_a_movie_as_actor(self):
        ...
        genre.save()
        director = Person(birthday=timezone.now(), name_surname=PersonTest.DIRECTOR_NAME)
        director.save()
        actor = Person(birthday=timezone.now(), name_surname=PersonTest.ACTOR_NAME)
        actor.save()
    ...
    def test_is_a_director_with_no_movies_as_director(self):
        non_director = Person(birthday=timezone.now(), name_surname=PersonTest.DIRECTOR_NAME)
        non_director.save()
